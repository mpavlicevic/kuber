...
function containerSettingsCheck(tree, containerName, container) {
  var out, type, ret;
  var realName = [tree.name, containerName].join('-');
  msg.delayedItemStart('Checking container settings for "' + realName + '"');
  var obj = spawn.exec('docker inspect ' + realName + ' 1>&2');
  if (obj.code === 0) {
    var inspect = JSON.parse(obj.stdout);
    ...
    for (var idx in container.ports) {
      var containerPort = [
        container.ports[idx].containerPort, 'tcp'
      ].join('/');
      var hostPort = container.ports[idx].hostPort;
      var netSettingsPort = inspect[0].NetworkSettings.Ports[containerPort][0];
      if (!(
        netSettingsPort &&
        netSettingsPort.HostPort &&
        netSettingsPort.HostPort == hostPort
      )) {
        out = 'changed';
        type = 'error'
        ret = false;
    ...
    for (var varName in container.env) {
      var varValue = container.env[varName];
      var combo = varName + '=' + varValue;
      var settingsVars = inspect[0].Config.Env;
      var found = false;
      for (var idxSet in settingsVars) {
        var check = settingsVars[idxSet];
        if (check === combo) {
          found = true;
          break;
        }
  ...
  if (type === 'error') {
    throw({
      msg: 'container configurations changed',
      mitigation: 'Remove the container using docker rm and start over'
    });
  }
  return ret;
...
exports.run = function(opts, tree) {
  ...
  for (var containerIdx in tree.startingOrder) {
    ...
    if (!containerCheck(tree, containerName, container)) {
      ...
      if (containerSettingsCheck(tree, containerName, container)) {
